...
- **Resolución de Destino de Envío para LIDs (`@lid`):** Al enviar mensajes de texto, notas de voz o archivos adjuntos a chats con identificador LID (`@lid`), el sistema resuelve automáticamente el número de teléfono real del contacto mediante `extractPhoneFromChat` (`remoteJidAlt`), evitando errores HTTP 400 Bad Request por números no existentes en WhatsApp.
- **Sincronización en Vivo y Polleo Automático:** Polleo periódico en segundo plano de la lista de chats (cada 5 segundos) y del historial de la conversación activa (cada 4 segundos) para actualizar inmediatamente la llegada de nuevos mensajes entrantes, insignias de no leídos y vistas previas en la barra lateral sin necesidad de recargar la página.
- **Soporte y Combinación de JIDs Múltiples (`@lid` y `@s.whatsapp.net`):** Al cargar o actualizar el historial de un chat, el sistema consulta en paralelo tanto los mensajes asociados al LID del dispositivo (`@lid`) como los asociados al número de teléfono (`@s.whatsapp.net`). Se unifican y deduplican todos los registros por ID de mensaje y se ordenan cronológicamente, garantizando que la entrada de mensajes recibidos desde el teléfono del tutor y las respuestas enviadas por la veterinaria se integren perfectamente en el mismo hilo de conversación.
